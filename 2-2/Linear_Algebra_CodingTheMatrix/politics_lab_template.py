...
def policy_compare(sen_a, sen_b, voting_dict):
    ...
    for i in range(len(voting_dict[sen_a])):
        dot+=voting_dict[sen_a][i]*voting_dict[sen_b][i]
    return dot
...
def most_similar(sen, voting_dict):
    ...
    """
    results = { policy_compare(sen,s1,voting_dict):s1 for s1 in voting_dict if s1 != sen}
...
def find_average_similarity(sen, sen_set, voting_dict):
    ...
    Example:
        ...
        True
    """
    return  sum([policy_compare(sen,s1,voting_dict) for s1 in sen_set])/len(sen_set)

# give the last name (or code that computes the last name)
most_average_Democrat = find_average_similarity('Chafee',vd,vd)
...
def find_average_record(sen_set, voting_dict):
    ...
    Example: 
        ...
        [0.0, 1.0, 1.0]
    """
    return [sum([voting_dict[sen][idx] for sen in sen_set])/len(sen_set) for idx in range(len(voting_dict[list(voting_dict.keys())[0]]))]
