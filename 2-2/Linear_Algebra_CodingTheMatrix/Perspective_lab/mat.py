...
def add(A, B):
    ...
    """
    assert A.D == B.D 
    w = { (i,j):(A[(i,j)] + B[(i,j)]) for i in A.D[0] for j in A.D[1] if (A[(i,j)] + B[(i,j)] != 0) }
    return Mat(A.D, w)
...
def scalar_mul(M, x):
    ...
    if (x==0):
        ...
        w = { (i,j):(M[(i,j)] * x) for i in M.D[0] for j in M.D[1] if M[(i,j)] != 0 }
        return Mat(M.D, w)
...
def transpose(M):
    ...
    True
    """
    w = { (j,i):(M[(i,j)]) for i in M.D[0] for j in M.D[1] if M[(i,j)] != 0 }
    return Mat((M.D[1], M.D[0]), w)
...
def matrix_matrix_mul(A, B):
    ...
    for s in A.D[1]:
        for r in A.D[0]:
            for t in B.D[1]:
                if ( ((r,s) in A.f) and ((s,t) in B.f) ):
                    w[(r,t)] = w[(r,t)] + A[(r,s)] * B[(s,t)]
    return w
